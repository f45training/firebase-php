...
class ApiException extends \RuntimeException implements FirebaseException
{
     ...
     */
    private $response;
    public static function wrapThrowable(\Throwable $e)
    {
        ...
        if ($response) {
            $message = isset(JSON::decode((string) $response->getBody(), true)['error']) ? JSON::decode((string) $response->getBody(), true)['error'] : $message;
        }
