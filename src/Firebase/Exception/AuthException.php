...
{
    public static $errors = [CredentialsMismatch::IDENTIFER => CredentialsMismatch::class, EmailExists::IDENTIFIER => EmailExists::class, EmailNotFound::IDENTIFIER => EmailNotFound::class, InvalidCustomToken::IDENTIFER => InvalidCustomToken::class, InvalidPassword::IDENTIFIER => InvalidPassword::class, MissingPassword::IDENTIFIER => MissingPassword::class, OperationNotAllowed::IDENTIFER => OperationNotAllowed::class, UserDisabled::IDENTIFER => UserDisabled::class, WeakPassword::IDENTIFIER => WeakPassword::class];
    /**
    ...
    public static function fromRequestException(RequestException $e)
    {
        ...
        $message = isset($errors['error']['message']) ? $errors['error']['message'] : $message;
        $candidates = array_filter(array_map(function ($key, $class) use($message, $e) {
            return false !== stripos($message, $key) ? new $class($e->getCode(), $e) : null;
        }, array_keys(self::$errors), self::$errors));
        $fallback = new static(sprintf('Unknown error: "%s"', $message), $e->getCode(), $e);
        return !empty(array_shift($candidates)) ? array_shift($candidates) : $fallback;
    }
}