...
class Firebase
{
     ...
     * @return Firebase
     */
    public function asUserWithClaims($user, array $claims = [])
    {
        ...
        return $this->auth ? $this->withCustomAuth(new UserAuth($this->auth->getUser($uid, $claims))) : $this->withCustomAuth(new CustomToken($uid, $claims));
    }
    /**
    ...
    private function createDatabaseClient(UriInterface $databaseUri)
    {
        ...
        $http = new Client(['base_uri' => $databaseUri, 'handler' => $stack, 'auth' => 'google_auth']);
    ...
    private function createGoogleAuthTokenMiddleware(ServiceAccount $serviceAccount)
    {
        ...
        $credentials = ['client_email' => $serviceAccount->getClientEmail(), 'client_id' => $serviceAccount->getClientId(), 'private_key' => $serviceAccount->getPrivateKey()];
